...
| `dispatch-wave` | Decide whether ≥2 artifacts should go to parallel agents, then brief them: six pre-flight checks (runtime resources included), nine mandatory brief fields including per-agent runtime isolation and cleanup, and a post-flight review that checks scope against the remote, not the report |
