...
- **Judged screens.** A judged surface is not done when its tests pass. The canonical source depends on the claim: appearance and composition resolve to the rendered frame on the target device; interaction to observed device behavior; accessibility to the accessibility tree plus behavior; implementation to source and tests. Before "done", a reviewer who did not build it and has not read its rationale reviews device captures of the whole screen (`kind: cold`), and a second review checks the built screen against the selected direction (`kind: conformance`). How much divergence a change owes is declared up front as `design_intent: preserve | refit | rethink`; only `rethink` owes three competing whole-screen concepts and a named human selection. Rule and protocol: `tools/blueprint/template/docs/methodology/judged-screen-pattern.md`.
